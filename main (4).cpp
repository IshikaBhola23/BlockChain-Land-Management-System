...
// list of all current lands in a system
vector<int> all_lands;
...
void Block::MineBlock()
{
    ...
    cout << "The root hash of the mined block:" << rootHash << endl
         << endl;
}
...
int main()
{
    ...
    while (userIP != 4) // user does not want to exit
    {
        ...
        {
            int seller, land;
            cout << "\nEnter Your Unique Seller Number and The Land you Own\n";
            cin >> seller >> land;
            if (find(all_lands.begin(), all_lands.end(), land) == all_lands.end())
            {
                all_lands.push_back(land);
                id_lands[seller].push_back(land);
                assignStakes(allTrans, seller);
                cout << "\nThe Land with ID " << land << " has been registered in the name of " << seller << "\n";
            }
            else
            {
                cout << "\nThe land you are trying to register belongs to someone else\n";
            }
        }
        else if (userIP == 2) // if the user want to carry out a transaction,i.e.,transfer of a land
        {
            ...
            {
                cout << "\nMining block " << transCount / 4 << "..." << endl;
        ...
        {
            int id = 0;
            cout << "\nEnter the Land ID whose details are to be viewed: \n";
            cin >> id;
    ...
    {
        cout << "\nNeed atleast 4 transactions to mine a block\n"
             << "Current Transactions: " << transCount % 4 << "\n";
    }
